...
char Respond_Buffer[8];
BYTE CommandIndex;
BYTE CurrentRotationNo;
BOOL IsRunning;
...
void UserInit(void)
{
	...
	Respond_Buffer[0] = 0x4F;
	Respond_Buffer[1] = 0x4B;
	Respond_Buffer[2] = 0x00;
...
{   
    ...
    if(USBUSARTIsTxTrfReady())
    {
		...
		if(numBytesRead != 0)
		{
			...
			*/
			...
			for(i=0;i<numBytesRead;i++)
			{
				...
				case 0x0D:
					...
					if(CommandIndex < 15)
					{
						Command_Buffer[CommandIndex] = 0x00;
						
						
						
						//*****************************************
						
						if(!strcmppgm2ram(Command_Buffer, "RPULSE"))
						{
						Rotate((BYTE)0);
						putUSBUSART("OK", length);
						}
						else if(!strcmppgm2ram(Command_Buffer, "LPULSE"))
						{
						Rotate((BYTE)1);
						putUSBUSART("OK", length);
						}
						else if(!strcmppgm2ram(Command_Buffer, "RUN"))
						{
						IsRunning = TRUE;
						putUSBUSART("OK", 2);
						}
						else if(!strcmppgm2ram(Command_Buffer, "STOP"))
						{
						IsRunning = FALSE;
						putUSBUSART("OK", 2);
						}
						...
						if(Command_Buffer[0] == 0x52 && Command_Buffer[1] == 0x50)
						{
							Rotate((BYTE)0);
							putUSBUSART("OK", length);
						}
						
						if(Command_Buffer[0] == 0x4C && Command_Buffer[1] == 0x50)
						{
							Rotate((BYTE)1);
							putUSBUSART("OK", 4);
						}
						if(Command_Buffer[0] == 0x52 && Command_Buffer[1] == 0x55)
						{
							IsRunning = TRUE;
							putUSBUSART("OK", 2);
						}
						if(Command_Buffer[0] == 0x53 && Command_Buffer[1] == 0x54)
						{
							IsRunning = FALSE;
							putUSBUSART("OK", 2);
						}
				...
				default:
					if(CommandIndex < 15)
					{
					Command_Buffer[CommandIndex] = USB_Out_Buffer[i];
					CommandIndex++;
					}
					break;
				}
			...
			*/
			if(USB_Out_Buffer[0] == 0x52 && USB_Out_Buffer[1] == 0x50 && USB_Out_Buffer[2] == 0x55 && USB_Out_Buffer[3] == 0x4C)
			{
				Rotate((BYTE)0);
				putUSBUSART(Respond_Buffer, 3);
			}
			else if(USB_Out_Buffer[0] == 0x4C && USB_Out_Buffer[1] == 0x50 && USB_Out_Buffer[2] == 0x55 && USB_Out_Buffer[3] == 0x4C)
			{
				Rotate((BYTE)1);
				putUSBUSART(Respond_Buffer, 3);
			}
			else if(USB_Out_Buffer[0] == 0x52 && USB_Out_Buffer[1] == 0x55 && USB_Out_Buffer[2] == 0x4E)
			{
				IsRunning = TRUE;
				putUSBUSART(Respond_Buffer, 3);
			}
			else if(USB_Out_Buffer[0] == 0x53 && USB_Out_Buffer[1] == 0x54 && USB_Out_Buffer[2] == 0x4F && USB_Out_Buffer[3] == 0x50)
			{
				IsRunning = FALSE;
				putUSBUSART(Respond_Buffer, 3);
			}
